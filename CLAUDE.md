...
4. Stores a compact structured review summary via `POST /api/memory/save` after Phase 5 cleanup
...
**Key constraints:**
- The MCP interface is read-only; writing uses the worker HTTP API on `localhost:37777` (default)
...
- The `/api/memory/save` endpoint is an internal claude-mem API; breakage across claude-mem versions causes graceful degradation (no-op), not errors
