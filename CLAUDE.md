...
skills/comprehensive-review/HELP.md                          ← --help text (output by comprehensive-review-help skill)
skills/comprehensive-review-help/SKILL.md                    ← dedicated --help skill (disable-model-invocation: true)
...
- **`--help` is implemented as a separate skill** (`skills/comprehensive-review-help/SKILL.md`) with `disable-model-invocation: true`. The skill uses `` !`cat "${CLAUDE_SKILL_DIR}/../comprehensive-review/HELP.md"` `` to output HELP.md directly — no LLM involved. The main skill's `--help` handling redirects users to `/comprehensive-review-help`. Do NOT attempt to handle `--help` inside the main `SKILL.md` via bash snippets, injection sentinels, or stop instructions — those approaches were tried in v1.8.1–1.8.6 and all failed because the LLM cannot reliably stop mid-document. `HELP.md` remains the single source of help text.
