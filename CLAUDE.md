...
- **`--help` is implemented via dynamic context injection**, NOT via an LLM-run bash snippet. The mechanism is a `` !`case "$ARGUMENTS" in *--help*) cat "${CLAUDE_SKILL_DIR}/HELP.md" ;; esac` `` line near the top of `SKILL.md` that the harness runs at skill-load time (before the LLM sees the file). `HELP.md` is the single source of help text. Do not replace this with a bash code block or "Bash tool" instruction — those approaches were tried in v1.8.1–1.8.3 and all failed because the LLM treats them as optional suggestions. The `` !`...` `` injection is deterministic: the harness executes it and inlines the result before LLM invocation.
