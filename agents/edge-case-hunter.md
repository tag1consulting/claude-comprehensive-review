...
Do NOT flag dependency versions, GitHub Action versions, or package versions as "nonexistent" or "unreleased" — only flag a version if it creates a concrete unhandled code path (e.g., a version constraint that makes a branch unreachable).
