...
5b. **Retrieve prior review history** (skip if MEM_AVAILABLE is false):
   ...
   - If the MCP tool fails (claude-mem not installed as MCP server), fall back to:
     `curl -sf "http://127.0.0.1:${MEM_PORT}/api/memory/search?q=Review+<REPO_SLUG>&limit=5" 2>/dev/null`
   - If results are returned, fetch full details via `mcp__plugin_claude-mem_mcp-search__get_observations`.
   - Condense results into a PRIOR_REVIEW_CONTEXT block (~500 tokens max):
     ```
     Prior reviews (last N):
     - 2026-03-15: 12 files, 1 Critical (auth bypass in handlers/auth.go:42), 3 High
     - 2026-03-01: 8 files, 0 Critical, 1 High (missing nil check in models/user.go:88)
...
curl -sf -X POST "http://127.0.0.1:${MEM_PORT}/api/memory/save" \
  ...
  -d "{\"text\": \"<summary>\", \"title\": \"Review: <REPO_SLUG> #<PR_NUMBER|branch_name> <YYYY-MM-DD>\", \"project\": \"<REPO_SLUG>\"}"
```
